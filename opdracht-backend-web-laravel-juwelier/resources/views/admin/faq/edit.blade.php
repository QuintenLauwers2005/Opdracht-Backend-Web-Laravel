        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form action="{{ route('admin.faq.update', $faq) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="question" id="question" value="{{ old('question', $faq->question) }}" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500">
                </div>

                <div class="mb-4">
                    <label for="answer" class="block text-gray-700 font-semibold mb-2">Antwoord *</label>
                    <textarea name="answer" id="answer" rows="6" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500">{{ old('answer', $faq->answer) }}</textarea>
                </div>

                <div class="mb-4">
                    <label for="order" class="block text-gray-700 font-semibold mb-2">Volgorde</label>
                    <input type="number" name="order" id="order" value="{{ old('order', $faq->order) }}" min="0" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500">
                </div>
